...
\section{Excitation power and spectral stability}
Comparing the mean normalized cross-correlation of sample 1510 from 4.7K and 20K in \ref{fig:histogram-of-normalized-cross-correlation12}, it is interesting that the spectral diffusion seems to be greater at lower temperature. Based on the hypothesis that the spectral diffusion is related to the band bending near the surface, this can only be explained when the band bending gets greater as temperature drops. As the temperature drops, less donors are ionized, which drives the bulk fermi level away from the minimum of conduction band energy, if it is matched with a pinned surface fermi level, the result would be a decrease of the band bending, which violates my observation. But when the surface state density is low (less than 10$^{11}/cm^{2}$), the  
